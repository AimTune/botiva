...
```
packages/
  core/{ts,go,dotnet,python}     # engine + protocol + ports (zero-dep in every language) — incl. the Authenticator port
  authentication/{ts,go,dotnet,python}  # Authenticator adapters: static-token, HMAC-JWT, cookie (PROTOCOL.md §2.1)
  server/
...
cd packages/authentication/python && PYTHONPATH=../../core/python python -m botiva_auth.selftest  # auth adapters + engine hook
python examples/python/server.py --selftest               # :8795 stdlib ws transport E2E incl. auth (examples add the 4 package roots to sys.path themselves)
...
- **auth port** — the fifth port, implemented in all four languages (PROTOCOL.md §2.1). The port (`Authenticator`/`AuthContext`/`AuthResult`/`AuthenticationError`/`AllowAllAuthenticator` + close code 4401) lives in **core** ([packages/core/ts/src/auth.ts](packages/core/ts/src/auth.ts), Go `auth.go`, .NET `Auth.cs`, Python `auth.py`); the transient `error` frame + `errorFrame()` helper live with the protocol. The engine option (`authenticator` / `Authenticator` / `authenticator=`) gates connect() **before any state is touched**: throw/raise/return-error `AuthenticationError` on reject, a returned userId overrides the client-asserted one (anti-spoofing), `claims` land in `TurnContext.meta.auth`. The three reference adapters — **StaticToken**, **HMAC-JWT** (HS256, stdlib crypto — verifies sig + exp/nbf), **Cookie** (composable; reads a named cookie from the forwarded headers, delegates to an inner verifier) — ship in `packages/authentication/<lang>`: `@botiva/authentication` (TS, own package), `github.com/aimtune/botiva/authentication` (Go, own module in go.work), `Botiva.Authentication` (.NET project in Botiva.slnx), `botiva_auth` (Python package). Transports extract the credential (`?token=` / `hello.token` / `Authorization: Bearer` / cookie header) and forward request headers; on reject they emit an `error` frame + WS close 4401 (Socket.IO `disconnect(true)`). The chativa client (`@chativa/connector-botiva`) sends `token` in its hello frame and stops reconnecting on a 4401/`error` frame (`onAuthError`). Selftests: TS `pnpm smoke` §10, Go `TestWebSocketAuthentication` + the authentication module tests, .NET `Botiva.SelfTest` + `Botiva.Example -- --selftest` (/chat-secure), Python `botiva_auth.selftest` + `examples/python/server.py --selftest`.
