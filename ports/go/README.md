...
```
botiva/                  engine, protocol, events, stores, DemoRuntime  (zero deps)
botiva/ws/               WebSocket transport — stdlib-only RFC 6455 server + client
adapters/langchaingo/    agent adapter for langchaingo llms.Model        (separate module)
examples/server/         runnable demo server (:8793) with --selftest
```
...
import (
    ...
    "github.com/chativa/botiva-go/botiva/ws"
)
...
`adapters/langchaingo` (its own module, so the core stays dependency-free)
...
```go
import lcadapter "github.com/chativa/botiva-go/adapters/langchaingo"
...
```sh
go test ./...                                # engine + ws transport (E2E over httptest)
go run ./examples/server                     # demo server on :8793 (PORT overridable)
go run ./examples/server --selftest          # + scripted WS client, exit 0/1

cd adapters/langchaingo && go test ./...     # agent adapter (scripted fake model)
```
