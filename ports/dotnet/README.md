...
```
Botiva/                engine, protocol, events, stores, DemoRuntime   (BCL only)
Botiva.AspNetCore/     WebSocket transport — app.MapBotiva("/chat", engine)
Botiva.Agents/         agent adapter for Microsoft.Extensions.AI IChatClient
Botiva.Example/        runnable ASP.NET Core demo server (:8797) with --selftest
...
Botiva.SelfTest/       console engine self-test, exit 0/1
```
...
```sh
dotnet run --project Botiva.Example -- --claude    # real Claude behind the same agent loop
```
...
```sh
dotnet run --project Botiva.SelfTest                 # engine self-test, exit 0/1
dotnet run --project Botiva.Example                  # demo server on :8797 (PORT overridable)
dotnet run --project Botiva.Example -- --selftest    # + scripted WS client, exit 0/1
dotnet run --project Botiva.Example -- --claude      # real Claude (needs key, see above)
```
